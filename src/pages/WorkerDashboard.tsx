...
const WorkerDashboard = () => {
  ...
  const [workerTasksState, setWorkerTasksState] = useState(workerTasks)
  const upcomingTasks = workerTasks.filter(task => task.status === 'approved' || task.status === 'pending')
  const completedTasksCount = workerTasks.filter(task => task.status === 'completed').length
