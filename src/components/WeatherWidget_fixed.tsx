...
const WeatherWidget: React.FC = () => {
  ...
  const getWeatherIcon = (condition: string) => {
    switch (condition) {
      case 'sunny': return <Sun className="h-5 w-5 text-yellow-600 dark:text-yellow-400" />
      case 'cloudy': return <CloudRain className="h-5 w-5 text-muted-foreground" />
      case 'rainy': return <CloudRain className="h-5 w-5 text-blue-600 dark:text-blue-400" />
      default: return <Sun className="h-5 w-5 text-yellow-500 dark:text-yellow-300" />
    }
  }
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-4">
        ...
        {weather ? (
        <div className={`mt-2 p-3 rounded border ${weather.condition === 'stormy' || weather.condition === 'rainy' ? 'bg-red-50 dark:bg-red-950/20 text-red-900 dark:text-red-100 border-red-200 dark:border-red-800' : 'bg-blue-50 dark:bg-blue-950/20 text-blue-900 dark:text-blue-100 border-blue-200 dark:border-blue-800'}`}>
